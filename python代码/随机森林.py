...
pd.DataFrame(data).to_excel('随机森林结果概率.xlsx',sheet_name='data',index=0)
...
pd.DataFrame(data_dict).to_excel('随机森林权重.xlsx',sheet_name='data',index=False)
...
for i in range(2):
    ...
    plt.plot(fpr, tpr,
                 lw=2, label="随机森林_ROC "+name[i]+" (area = %0.2f)" % auc)
